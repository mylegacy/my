...
{
        .id =           "usbd480fb", /* ok */
        .type =         FB_TYPE_PACKED_PIXELS, /* ok */
//        .visual =       FB_VISUAL_PSEUDOCOLOR, /* used */
        .visual =       FB_VISUAL_TRUECOLOR, /* suggestec */
        .xpanstep    =     1, /* ok */
        .ypanstep    =     1, /* ok */
        .ywrapstep    =    1, /* ok */
        .line_length  = SCREEN_WIDTH * BYTES_PER_PIXEL, /* new */
        .accel =        FB_ACCEL_NONE, /* ok */
};
...
// maybe .fb_setcolreg = fb_setcolreg,
...
/* new */    unsigned int  memsize;
/* new */    unsigned char * videoMem = NULL;
   
/* new */    memsize    = SCREEN_WIDTH * SCREEN_HEIGHT * BYTES_PER_PIXEL;
/* new */    videoMem   = vmalloc(memsize);

    memset(videoMem, 0x0f, memsize);

    xxxfb_fix.smem_start = (unsigned long) videoMem;
    xxxfb_fix.smem_len   = memsize;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
    ...
    dev->vmemsize = memsize;
    dev->vmem = videoMem;
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

    memset(dev->vmem, 0, dev->vmemsize);

    info = framebuffer_alloc(sizeof(struct xxxfb_drv), NULL);
...
/* new */    info->screen_base = (char __iomem *) videoMem;
/* new */    info->fbops       = &usbd480fb_ops;
/* new */    info->var         = xxxfb_var;
/* new */    info->fix         = xxxfb_fix;
/* new */    info->flags       = FBINFO_FLAG_DEFAULT;
...
/* 628 */
/* new */    main_dev             = info->par;
/* new */    main_dev->info       = info;
/* new */    main_dev->scr        = kzalloc(memsize, GFP_KERNEL);
/* new */    info->pseudo_palette = main_dev->pseudo_palette;
    ...
    info->fix.smem_start = (unsigned long) videoMem;
...
/*new */  retval = register_framebuffer(main_dev->info);
